...
> **참고**: 이 파일은 문서용입니다. 실제 스케줄 동작은 `claude.ai/code/scheduled`의 트리거 프롬프트가 결정합니다.

---

## 1. 데일리 브리핑 + 조건부 섹터 리포트 (평일 07:00 UTC / 16:00 KST)
...
3단계로 동작:
- **STEP A**: 데일리 브리핑 (항상)
- **STEP B**: 섹터 리포트 (업종 등락률 ±5% 이상일 때만, 최대 2개)
- **STEP C**: 후처리 (사이드바/사이트맵/홈 업데이트 + 커밋/푸시)

상세 프롬프트는 `claude.ai/code/scheduled`의 트리거에 직접 등록되어 있음.
